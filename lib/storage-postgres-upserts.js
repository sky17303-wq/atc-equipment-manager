...
async function upsertPostgresRepairTicket(client, ticket) {
  await client.query(
    `INSERT INTO repair_tickets (
      id, inspection_id, application_id, item_id, quantity, issue_type,
      status, returned_to_rentable, note, created_by, created_at,
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      status = EXCLUDED.status,
      returned_to_rentable = EXCLUDED.returned_to_rentable,
      note = EXCLUDED.note,
    ...
    [
      ticket.id,
      ticket.inspectionId || null,
      ticket.applicationId || null,
      ticket.itemId,
      Number(ticket.quantity || 0),
      ticket.issueType || "damaged",
      ticket.status || "open",
      Number(ticket.returnedToRentable || 0),
      ticket.note || "",
      ticket.createdBy || "운영담당자",
...
async function upsertPostgresNotification(client, notification) {
  await client.query(
    `INSERT INTO notifications (
      id, type, channel, recipient, subject, body, status,
      related_id, dedupe_key, attempts, error, created_at, sent_at, updated_at
