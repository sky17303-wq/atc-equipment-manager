...
const { normalizeEmail, formatDate, addDays } = require("./utils");
const { buildLabels, buildStats, buildPeriodReport, calculateAvailability } = require("./domain");
...
const { handleRepairsApi } = require("./api-repairs");
const { handleInventoryApi } = require("./api-inventory");
const { handleNotificationsApi } = require("./notifications");
...
async function handleApi(req, res, url) {
  ...
  // 기간별 운영 리포트: 운영진(staff/admin/auditor)만 조회 가능. 기본 기간은 최근 30일.
  if (req.method === "GET" && url.pathname === "/api/reports") {
    ...
    if ((rawStart && !isValidDate(rawStart)) || (rawEnd && !isValidDate(rawEnd))) {
      sendJson(res, { error: "startDate/endDate는 YYYY-MM-DD 형식이어야 합니다." }, 400);
      return true;
    }
    const endDate = rawEnd || formatDate(new Date());
    const startDate = rawStart || formatDate(addDays(new Date(`${endDate}T00:00:00Z`), -29));
    if (startDate > endDate) {
      sendJson(res, { error: "startDate는 endDate보다 이후일 수 없습니다." }, 400);
      return true;
    }
    sendJson(res, buildPeriodReport(seed, startDate, endDate));
    return true;
  }
  ...
  if (await handleRepairsApi(context)) return true;

  if (await handleInventoryApi(context)) return true;

  if (await handleNotificationsApi(context)) return true;
