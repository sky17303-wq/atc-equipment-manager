...
## 6차: 모듈화, 보안, 운영 기능 (2026-07)

반영됨:

- `server.js` 단일 파일을 `lib/` 17개 모듈로 분리 (config/storage/auth/domain/api 계층)
- 조회 API 로그인 필수화, applicant 본인 데이터 범위 제한, 타 기관 예약 마스킹
- ERP 세션 토큰 `expires_at` 누락 시 거부 (만료 검사 우회 차단)
- 다품목 신청의 품목별 부분 검수 — 전 품목 검수 완료 시에만 신청 종결
- 파손/수리 검수 시 수리 티켓 자동 생성 (open → in_repair → resolved/scrapped, 재고 복귀)
- 알림 큐 + SMTP 발송 디스패처 (승인/반려/반출/반납/검수, 반납 임박·연체 스윕)
- 무의존성 SMTP 클라이언트 — SMTP 미설정 시 skipped 기록만 (카카오 알림톡은 스텁)
- 라벨에 실제 QR 이미지(SVG) 생성, 인쇄 30mm
- QR 카메라 스캔 화면 (BarcodeDetector, 미지원 브라우저는 수동 입력 폴백)
- 엑셀(.xlsx) 직접 업로드 일괄 반영 (브라우저 파싱, 한국어 헤더 자동 매핑)
- 기간별 운영 리포트 (사용률/파손율/연체/수리 티켓)
- DB 일일 백업 스크립트(`scripts/backup-db.sh`) + 로컬 배포 스크립트(`scripts/deploy.ps1`)

운영 전 추가 대상:

- 카카오 알림톡 실발송기 (KakaoLog 스텁 → NHN Cloud/Solapi 연동)
- Android 실기기 카메라 스캔·인쇄 라벨 스캔 실물 테스트
- Supabase 신규 사용자 승인 정책 화면
